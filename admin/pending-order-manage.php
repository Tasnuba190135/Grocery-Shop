...
// Check if form is submitted for processing or completing an order
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['processing'])) {
        updateOrderStatus($_POST['processing'], 'processing');
    } elseif (isset($_POST['completed'])) {
        updateOrderStatus($_POST['completed'], 'completed');
    }
}
...
<?php include_once'navbar.php'; ?>
    ...
    <table>
        ...
        $sql = "SELECT * FROM tbl_order WHERE status = 'pending' OR status = 'processing'";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            // Output table header
            echo "<table>";
            echo "<tr><th>Order ID</th><th>Status</th><th>User ID</th><th>Products(Quantity)</th><th>Created</th><th>Last Modified</th><th>Action</th></tr>";
